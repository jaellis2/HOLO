function [phiHO, ephiHO, jHO] = running_HOk(phi, k, data, Np_update, newfilename)
...
lin_source = (data.csig_s + (1./k)*data.nu_f.*data.sig_f).*phi + data.q; % LINEAR NEUTRON DENSITY
...
worked = addSource(newfilename, vol_source, Np_update, data);
